...
window.doRegister = function() {
    ...
    if (users.find(u => u.email === email)) { showToast('Этот email уже используется', 'error'); return; }
    const newUser = { username, password, email, name, role: users.length === 0 ? 'admin' : 'user', xp: 0, joinedAt: new Date().toISOString() };
    users.push(newUser);
    saveUsers(users);
...
window.openProfileModal = function() {
    ...
    const userEmail = getUsers().find(u => u.username === currentUser.username)?.email || '';
    ...
    document.getElementById('profileModalContent').innerHTML = `
        <div class="profile-header">
            ...
            <div>
                ...
                ${userEmail ? `<div style="font-size:0.85rem;color:var(--text-light);margin-top:4px"><i class="fas fa-envelope" style="color:var(--primary-blue)"></i> ${escHtml(userEmail)}</div>` : ''}
